...
CREATE TABLE IF NOT EXISTS `SCICONF`.`paper` (
  ...
  INDEX `fk_paper_paper_status1_idx` (`paper_status_idpaper_status` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_paper_section1`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_paper_paper_status1`
    FOREIGN KEY (`paper_status_idpaper_status`)
    REFERENCES `SCICONF`.`paper_status` (`idpaper_status`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
